 ...
 * Returns raw Uint8Array buffers for high-performance byte transfer.
 */
export async function readApp(args) {
    ...
    try {
        ...
        // Concatenate all chunks into single Node.js Buffers
        ...
        // Since Buffer is a subclass of Uint8Array, we can cast them directly.
        // This keeps the data as raw bytes for the viewbox without Base64 overhead.
        const stdoutRaw = new Uint8Array(stdoutBuffer);
        const stderrRaw = new Uint8Array(stderrBuffer);
...
/**
 * writeApp(args) -> { pid, bytesWritten }
 * Writes raw bytes (Uint8Array) to the stdin of a child process.
 * Standardizes on the single-argument object pattern used in the rest of the module.
 */
export async function writeApp(args = {}) {
    const { pid, data } = args;
    ...
    try {
        ...
        if (!child.stdin) {
            return errorResponse("Child process stdin is not available (check if stdio was ignored)");
        }

        // Check for Uint8Array (which includes Node Buffers)
        if (!(data instanceof Uint8Array)) {
            return errorResponse("Data must be a Uint8Array or Buffer for raw byte transfer");
        }

        // Node's writable stream .write() accepts Uint8Array directly.
        // It pushes the raw bytes into the pipe for the C/C++ child to read via stdin.
        const drained = child.stdin.write(data);

        // Handle backpressure if the pipe's internal buffer is full
        if (!drained) {
            await new Promise((resolve) => child.stdin.once('drain', resolve));
        }

        debugLog(`[WRITE-APP] Wrote ${data.byteLength} bytes to PID ${pid}`);
        ...
        errorLog(`[WRITE-APP] Error: ${err?.message ?? err}`);
        return errorResponse(err?.message ?? "write-app failed");
    }
}
